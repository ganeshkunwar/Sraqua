...
const EDITPRODUCTS_URL = '/editProducts';
...
const EditProduct = () => {
    ...
    useEffect(() => {
        ...
        const getProduct = async () => {
            try {
                const response = await axios.get(`products/${pId}`, {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await axiosPrivate.put(EDITPRODUCTS_URL,
                JSON.stringify({ id:pId, productName, image, category, price, stock }),
                {
        ...
        <div className=' w-max h-max flex flex-col items-center justify-center my-5 max-lg:my-5 shadow-xl p-5'>
            ...
            <div className="form">
                <form action="POST" className="flex flex-col justify-center items-center" onSubmit={handleSubmit}>
                    ...
                    <input type='submit' className={"w-60 mt-7 bg-blue-950 text-white transition-all duration-500 p-3 cursor-pointer"} value={"Edit Product"} onClick={handleSubmit} />
                </form>
            </div>
        </div>
    )
